...
describe('hedge regex is first-person only (2026-09-12)', () => {
  // A wake-up message about a neighbour's death tripped "(?:i )?couldn't get"
  // and bare "unfortunately" after the real work was done; the nudge made
  // Qwen re-send the notification twice.
  test.each([
  ...
  test.each([
    'I couldn\'t find the file anywhere in the workspace.',
    'Unfortunately I wasn\'t able to reach the camera.',
    'I don\'t have access to that calendar.',
  ])('first-person giving up still is: %s', (text) => {
    expect(HEDGE_GIVE_UP.test(text.toLowerCase())).toBe(true);
  });
});
