...
export default class NotificationsHandler extends BaseSkillHandler {
  ...
  private async sendNotification(toolCall: ToolCall, ctx: SkillHandlerContext): Promise<ToolResult> {
    try {
      ...
      // Auto-attach: if model didn't pass image_ids, check for images
      // generated by this choom in the last 30 minutes and attach them.
      if (imageIds.length === 0 && ctx.choomId) {
        const thirtyMinAgo = new Date(Date.now() - 30 * 60 * 1000);
        const recentImages = await prisma.generatedImage.findMany({
          where: { choomId: ctx.choomId, createdAt: { gte: thirtyMinAgo } },
        ...
        if (recentImages.length > 0) {
          ...
          console.log(`   📎 Auto-attached ${imageIds.length} recent image(s) from choom ${ctx.choomId}`);
        }
      }
