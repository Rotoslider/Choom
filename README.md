...
- **Execution Traces & Nightly Doctor**: Every agentic loop execution writes a structured JSON trace capturing tool calls (with duration, fine-grained error class — auth, rate_limit, timeout, network, upstream_4xx/5xx, template, permission_block, blocked_reissue and the original classes — parallel flag), nudges, fallbacks, compaction events, per-phase wall-clock (LLM prefill vs decode vs tool execution vs prep), and token usage including the largest single-call prompt. A nightly diagnostic job (22:00) analyzes traces (reclassifying historical ones with the same patterns), computes daily aggregates, separates contract-gate blocks and disabled-tool reissues from real failures so protection working never reads as breakage, reports a time split showing where the wall-clock went with drill-in pointers to the slowest single LLM call and biggest prompt, detects anomalies, and sends a report via Signal
