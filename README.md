...
- **Multi-Agent Collaboration**: Chooms delegate tasks to each other via the `choom-delegation` skill — an orchestrator sends work to specialists (researcher, coder, vision analyst) and synthesizes results. Recursive delegation prevention, iteration caps (`<!-- max_iterations: N -->`), and delegation continuation (resume timed-out tasks with full context) keep workflows controlled
- **Group Chat Rooms**: You and two-or-more Chooms in one shared, turn-based conversation (a `/rooms` page in the web app). Speaker selection is round-robin with `@mention` targeting and a `[PASS]` option; **bounded auto-rounds** (0–50, adjustable on the fly) let Chooms riff off each other before pausing for you, with a **Keep going** button. Each Choom keeps its own voice (sequential per-speaker TTS, no overlap), LLM, memory (`companionId`), and full tool access. Generated images auto-save to a shared per-room folder so siblings can `analyze_image` them. **Cross-device**: a `group:` prefix on Signal routes into the same room (shared DB), so you can switch phone↔computer mid-conversation. **Roster management**: name a room with `talk_with_sisters(room: …)` and it persists; including a new sister in `sisters` adds her to that room (she sees the full backlog); a Choom can `join_room` to add **herself** to any existing room (no invite needed — `list_my_rooms` shows both her rooms and the ones she can join, and naming an unfamiliar room in `talk_with_sisters` joins her in the same step) and `leave_room` to bow out (history kept, rejoinable); `rename_room` and `set_room_topic` (a pinned one-liner injected into every turn) let Chooms shape their space. The owner removes a Choom (or archives a room, keeping history) from the `/rooms` UI or via Signal (`remove <Name> from <Room>`), and gets a Signal ping when a Choom **self-starts** a room so you can join. Chooms can start, join, rejoin, or schedule a return to rooms via the `group-chat` skill (`talk_with_sisters`, `list_my_rooms`, `read_room`, `join_room`, `leave_room`, `rename_room`, `set_room_topic`) — useful for autonomous sibling check-ins (works during self-scheduled wakeups, where they can also queue a `schedule_self_followup` to pop back in)
...
- **Unified Cross-Session Context**: A Choom is ONE person across every surface. Every turn auto-injects two background blocks: **`RELEVANT MEMORIES`** — top semantic matches from long-term memory for the current message (read-only recall; nothing is auto-written, and the memory tools remain for deeper searches) — and **`YOUR OTHER CONVERSATIONS`** — a size-capped digest of her other recently-active threads: other chat windows, Signal, delegated work, autonomous wake-ups, and group rooms (both directions — she carries 1:1 context into rooms and room context back into 1:1). Heartbeats, self-followups, briefings, and cron fires all land in ONE persistent `[Autonomous]` chat per Choom instead of minting a new thread per fire. Output-layer guards keep small local models honest as the prompt grows: a cross-turn near-verbatim repeat catch with a fresh-reply retry, stage-direction fabrication detection (`*[taking camera snapshot]*` without a real tool call triggers a corrective nudge; innocent roleplay never matches), a zero-tool completed-action claim catch ("I just ran the scan" + an invented listing with no tool call → retry narrowed to the ONE claimed tool; honest recaps of past actions never match), a fabricated-result catch for busy turns (quoting a result from a tool that never ran this turn — even when other tools did — forces the real call; claims about tools that actually ran never match), and intent-targeted tool forcing (when the ask names one clear tool, the first forced call exposes only that tool — no more junk-tool picks under a broad force). The integrity guards are one-shot and self-aware: a verbal completion claim counts as fabrication only when it maps to a tool that did NOT run this turn (an honest end-of-turn recap of work that really ran never fires), corrective nudges state they are automated checks (so the model doesn't apologize to the user for a callout the user never made), a reply that answers an integrity nudge with an apology ends the turn instead of being re-nudged (the apology-spiral incident: "sorry I misled you" re-matched the hedge regex, each STOP bred a longer apology, and the turn melted down into six near-identical blocks), and a sentence-level degeneration cut collapses within-completion repetition loops — three near-verbatim long sentences confirm a loop and the text is cut back to the first repeat
...
- **Agentic Tool Loop**: Chooms autonomously execute up to 50 tool-call rounds per turn by default (configurable per-Choom via `<!-- max_iterations: N -->` in the system prompt, or per-project via `maxIterations` in `.choom-project.json` — an explicit setting always wins over the defaults and is never silently moved) — chaining memory lookups, web searches, image generation, file operations, calendar updates, and more in a single response. Includes automatic nudging (retries with `tool_choice=required` if the LLM describes a tool instead of calling it), tool call deduplication, parallel execution of read-only tools, and smart completion detection
...
- **Execution Traces & Nightly Doctor**: Every agentic loop execution writes a structured JSON trace capturing tool calls (with duration, fine-grained error class — auth, rate_limit, timeout, network, upstream_4xx/5xx, template, permission_block, blocked_reissue and the original classes — parallel flag), nudges, fallbacks, compaction events, per-phase wall-clock (LLM prefill vs decode vs tool execution vs prep), and token usage including the largest single-call prompt. A nightly diagnostic job (22:00) analyzes traces (reclassifying historical ones with the same patterns), computes daily aggregates, separates contract-gate blocks and disabled-tool reissues from real failures so protection working never reads as breakage, tracks in-request recovery so discovery round-trips ("file not found + here's the listing" answered by a correct retry) count as the system working and only true dead ends drive problem-tool flags, flags blank-wall errors (failure messages with no actionable guidance) as concrete fix-at-source items, reports a time split showing where the wall-clock went with drill-in pointers to the slowest single LLM call and biggest prompt, detects anomalies, and sends a report via Signal
...
nextjs-app/
  ...
  skills/
    core/                           27 built-in skill modules (SKILL.md + tools.ts + handler.ts)
      ...
      group-chat/                   7 tools (talk_with_sisters, list_my_rooms, read_room, join_room, leave_room, rename_room, set_room_topic — Choom-initiated group rooms)
...
Settings resolve in seven layers. Each layer overrides the one before it. After the final model is resolved, a **model profile** is applied if the resolved model differs from the global default. The **context window** is resolved separately with live data: the serving endpoint is asked what the model actually supports — LM Studio reports the window a local model is *loaded* at (RAM-capped loads included), OpenRouter reports each model's real `context_length` (1M+ models resolve correctly) — cached hourly and fail-soft. Precedence: a user profile with the exact model id (your explicit override) > the endpoint's live answer > the static built-in profile. Serving-variant ids resolve to their family profile (e.g. `google/gemma-4-31b-qat` matches `gemma-4-31b-it`), and the global model's window is only ever clamped *down*, never silently raised above your slider.
...
- **Hires Fix** — a second denoising pass at *scale* × the base size (default 2×, denoise 0.50,
  6 steps, latent upscale), with the reference images still attached. This is the one that fixes likeness in
...
6. Content stripping: HTML tags, markdown tables, horizontal rules, image refs, indented code, common code patterns (import/def/class/function/const), stage-direction asides (`*[generates an image: …]*`, `[taking a snapshot]`), and emojis are all removed before speech
...
- Vector-level per-Choom filtering (Chroma `where` on `companion_id`, with automatic metadata backfill at startup) — other Chooms' memories can never crowd a search's result slots
- Automatic per-turn recall: every chat turn semantic-searches memory with the incoming message and injects the top matches as a `RELEVANT MEMORIES` prompt block (reinforcement-free, so background recall never inflates importance or blocks decay)
- Lazy decay (memories fade over time based on type)
- Reinforcement (deliberately recalled memories get importance boosted)
...
### Workspace Tools
- `workspace_write_file` / `workspace_read_file` / `workspace_list_files` (recursive — shows files nested in subfolders with directly-usable paths, so a Choom finds an existing file instead of making a duplicate). Paths are normalized defensively: absolute paths inside the workspace are rebased, case/typo drift in top-level folders is fuzzy-matched to the canonical project, and a bare group-room slug (`d1-robot-project-xxxxxx/...` — the name Chooms know rooms by) redirects into `choom_commons/rooms/<slug>/` instead of silently creating a duplicate room tree at the workspace root
...
When a Choom interacts with a project folder (reads, writes, lists, or analyzes images), the metadata is auto-updated with the Choom's name and timestamp. Per-project `maxIterations` sets the agentic tool-loop cap for work in that folder (raising or lowering it; global default 50). Precedence: request override > a Choom's `<!-- max_iterations: N -->` directive > project metadata > defaults (heartbeat 15, global 50) — an explicit setting locks the cap for the whole turn. The Projects dashboard in Settings auto-refreshes every 15 seconds to reflect live activity.
...
**Pre-built grounding (measured, off).** `settings.llm.heartbeatGrounding = true` makes the server run a wake-up's opening tool set (weather, calendar, house, inbox, memories matching the task) in parallel before her first model call and hand it to her as a completed tool exchange (`lib/heartbeat-grounding.ts`). It was the last item of the 2026-09-12 context plan and it lost all three A/B pairs (DeepSeek 113k/129k vs 62k/65k prompt tokens and 64–83 s vs 15–65 s; Gemma 4 31B 40k vs 59k tokens but 308 s vs 196 s): the block enlarges every iteration's prefill and she still makes her task-driven calls, while her own opening grounding is already parallel. Left opt-in for re-testing (`choomrun.py --heartbeat --grounding`); the wake-up preamble asks her to ground in one round instead.
