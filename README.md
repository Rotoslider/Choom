...
- **Group Chat Rooms**: You and two-or-more Chooms in one shared, turn-based conversation (a `/rooms` page in the web app). Speaker selection is round-robin with `@mention` targeting and a `[PASS]` option; **bounded auto-rounds** (0–50, adjustable on the fly) let Chooms riff off each other before pausing for you, with a **Keep going** button. Each Choom keeps its own voice (sequential per-speaker TTS, no overlap), LLM, memory (`companionId`), and full tool access. Generated images auto-save to a shared per-room folder so siblings can `analyze_image` them. **Cross-device**: a `group:` prefix on Signal routes into the same room (shared DB), so you can switch phone↔computer mid-conversation. **Roster management**: name a room with `talk_with_sisters(room: …)` and it persists; including a new sister in `sisters` adds her to that room (she sees the full backlog); a Choom can `leave_room` herself (history kept, re-invitable); `rename_room` and `set_room_topic` (a pinned one-liner injected into every turn) let Chooms shape their space. The owner removes a Choom (or archives a room, keeping history) from the `/rooms` UI or via Signal (`remove <Name> from <Room>`), and gets a Signal ping when a Choom **self-starts** a room so you can join. Chooms can start, rejoin, or schedule a return to their own rooms via the `group-chat` skill (`talk_with_sisters`, `list_my_rooms`, `leave_room`, `rename_room`, `set_room_topic`) — useful for autonomous sibling check-ins (works during self-scheduled wakeups, where they can also queue a `schedule_self_followup` to pop back in)
